...
public abstract unsafe class SymbolResolver
{
    ...
    public void* Resolve(string name) 
        => TryResolve(name, out void* address) 
            ? address 
    ...
    public bool TryResolve<T>(string name, out T* address) where T : unmanaged
    {
        address = (T*)GetAddressOrZero(name);
        return address != null;
    }

    public T* Resolve<T>(string name) where T : unmanaged
        => TryResolve(name, out T* address) 
            ? address
            : throw new UnresolvedSymbolException(name);

    public unsafe T* Resolve<T>(params string[] names) where T : unmanaged
    {
        foreach (string name in names)
        {
    ...
    public unsafe bool TryResolveFirstMatch<T>(Regex regex, out T* address) where T : unmanaged
    {
    ...
    public unsafe T* ResolveFirstMatch<T>(Regex regex) where T : unmanaged
        => (T*)ResolveFirstMatch(regex);
}